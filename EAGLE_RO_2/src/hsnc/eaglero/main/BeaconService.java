...
//		@Override
	...
	/** 아까도 연결 범위 내에 있었고 지금도 범위 내에 있는 지를 확인하는 boolean 
	 */
	private static boolean oneTimeCoupon = false;
	private static boolean isStillNearby	= false;
...
//		@Override
		public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
			...
			if(device.getAddress().equals("D0:FF:50:67:6F:96")){
				...
				BeaconEvent be = new BeaconEvent();
				int j	= 0;
				...
				if(loginCounter == 9){
					...
					if(Math.abs(j / 10 ) < 300){
						if(isStillNearby){
							System.out.println("아까부터 계속 로그인");
							//TODO 여전히 연결된 상태일때 작동할 내용
							if(Math.abs(j / 10 ) < 170){
								if(oneTimeCoupon){
							...
							System.out.println("처음으로 로그인");
							//TODO 처음 연결된 상태일 떄 작동할 내용
							
							be.message(1);
							isStillNearby	= true;
						}
					}else{
						//TODO 연결 끊겼을 때 작동할 내용
						isStillNearby	= false;
						oneTimeCoupon = false;
					}
				}
